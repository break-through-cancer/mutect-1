...
//for merging vcfs 
if (!params.containsKey('make_force_calling_intervals'))  params.make_force_calling_intervals = false
...
process BGZIP_AND_INDEX_VCF {
    ...
    input:
    tuple val(pairName), path(vcf)

    output:
    tuple val(pairName), path("${pairName}.filtered.vcf.gz"), path("${pairName}.filtered.vcf.gz.tbi"), emit: gz_vcf
    ...
    test -s !{pairName}.filtered.vcf.gz.tbi
    '''
}
...
process merge_all_sample_vcfs {
  ...
  mkdir -p tumor_only_vcfs normalized_vcfs
  ...
  for f in \$(find . -maxdepth 1 -type f -name '*.filtered.vcf.gz' -print | sort); do
    ...
    tumor_only="tumor_only_vcfs/\$(basename "\${f}" .vcf.gz).tumor_only.vcf.gz"
    ...
    tabix -f -p vcf "\${tumor_only}"

    # Left-align indels and split multiallelics so the same real variant
    # from different samples collapses to one consistent representation
    # rather than surviving as near-duplicate records after merge.
    norm_out="normalized_vcfs/\$(basename "\${tumor_only}" .vcf.gz).norm.vcf.gz"
    ...
    tabix -f -p vcf "\${norm_out}"
    echo "\${norm_out}" >> tumor_vcfs.list
  done
...
process VCF_TO_INTERVAL_LIST {
    tag "merged_force_calling_intervals"
    label 'process_low'
    container "broadinstitute/gatk:4.5.0.0"
    publishDir "${params.outdir}/force_calling", mode: 'copy'
    errorStrategy 'retry'
    maxRetries 2
    ...
    output:
    path "force_calling.interval_list", emit: interval_list
...
workflow {
    ...
    // -----------------------------------------------------------------------
    // Merge per-sample PASS VCFs, then derive a force-calling interval list
    // -- only runs when explicitly enabled via params.make_force_calling_intervals
    // -----------------------------------------------------------------------

    if (params.make_force_calling_intervals) {

        per_sample_pass_vcfs =
            GATHER_AND_FILTER
                .out
                .pass_vcf_getzlab
                .map { vcf -> tuple(vcf.baseName.replaceAll(/\.MuTect1\.PASS$/, ''), vcf) }

        BGZIP_AND_INDEX_VCF(per_sample_pass_vcfs)

        gz_vcfs_ch =
            BGZIP_AND_INDEX_VCF
                .out
                .gz_vcf
                .map { pairName, vcf, tbi -> vcf }
                .collect()
